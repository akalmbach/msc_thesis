...
\begin{figure}
	...
	\caption{
		PDFs of Dirichlet distributions with 2D symmetric parameters $\alpha$. These can be used as a prior over the 2D PMFs. Note that this means there are $D-1 = 1$ degree of freedom, \emph{ie.} this is a prior for a Bernoulli random variable and this case is identical to a $\mathrm{Beta}(\alpha_0, \alpha_1)$ distribution. We can imagine each of these curves as giving a distribution of probabilities for coins from an unfair coin factory, where the average over all coins looks fair, but (for large $\alpha$) most individual coins are very unfair.
		% \todo[inline]{Remove analogy? Place in text?}
	}
	\label{fig:beta-pdf}
