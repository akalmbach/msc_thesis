...
\todo{I think it would be appropriate to make the connection to the large and even earlier literature on multi-dimensional scaling (which includes PCA and many other linear and non-liner methods) which is arguably a form of topic modeling, but unarguably a close conceptual relative.}

Although PLSA is extremely general, with no further assumptions topic models are unlikely to find intuitive topics and also often generalize poorly as they are prone to overfitting. \todo{This correct assertion needs a citation}
Latent Dirichlet Allocation improves this situation, while minimally decreasing the expressive power of the model in practice by making a further assumption, namely, placing independent Dirichlet priors on each $\theta_d$ and $\phi_z$.\todo{Did you already EXPLICITLY define $\theta_d$ $\phi_z$ ? I don't think so, even though you kinda defined the sets they belong to.}   Recall that the Dirichlet distribution $Dir(\theta; \boldsymbol{\alpha})$ is the multivariate generalization of the Beta distribution, i.e. a distribution over the $K-1$ dimensional simplex with parameter $\boldsymbol{\alpha} \in \mathbb{R}^K$. In other words \todo{comma} it is a distribution over the $K$ dimensional discrete-valued PMFs. \todo{Did you already define PMF? I don't think so.} To be exact the, Dirichlet distribution is a (conjugate)prior for categorical and multinomial distributions, with PDF: \todo{Did you already define PMF? I don't think so.} 
\begin{equation}
...
\todo{What is gamma? What does alpha mean?  These "obvious" things should be as explicit as possible.}
...
\todo{Why are you using $\times$ here?}

Various other methods for approximate posterior inference for LDA have been explored, particularly variational methods and more recently stochastic gradient approximations to variational methods (For a comprehensive review, see \citep{geigle2016}). Despite the efficiency of these methods, they lack the appealing anytime,\todo{maybe say "anytime (in the sense of Zilberstein)"} online aspect of a sampling based approach, and are much less flexible to modification of the model. For these reasons in this work we rely only on the Gibbs sampling based approach.
