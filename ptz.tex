...
Fig.~\ref{fig:vs_vqvae} shows the RMSE for observed topic encodings and predicted topic priors compared to the real images at the corresponding views. They reflect our qualitative assesment that our method performs approximately as well as the VQ-VAE. However, despite our approach and the VQ-VAE having autoencodings that are clearly qualitatively better than the CAE, the distributions of RMSEs are surprisingly similar. This indicates that at this level of compression, RMSE is an imperfect performance measure.
...
\begin{figure}
    ...
    \end{center}
    \caption{ Performance comparison of our topic-based autoencoder and spatial prediction system ($\theta$-Map) with a VQ-VAE and a standard CAE.
    \protect\subref{fig:vs_vqvae_obs} Reconstruction performance of observed images (autoencoding with a very small bottleneck),
    \protect\subref{fig:vs_vqvae_pred} View predictions performance after training with 50 uniform random pan-tilt-zoom poses. The shaded region in this violin plot represents the kernel density estimate (Scott bandwidth) for each method, while the extrema and medians are represented by the horizontal ticks.
...
\todo[inline]{The role of the dropout layer}
\todo[inline]{A true variational categorical autoencoder}
\todo[inline]{Using geometry more to our advantage (bi-linear interpolation is great)}
\todo[inline]{Supervised learning for a feature function}

\subsection{Active learning}\label{sec:active-learning}
As a complimentary task to learning a view prediction model, we also spent significant effort on the problem of \emph{efficiently} learning the view prediction model, that is, trying to accurately predict views in novel locations with as few training observations as possible. There is a rich literature on efficient online learning through careful spatial sampling, including classic work such as \citep{Seo2000,guestrin2005near}, work on image mosaicing such as \citep{sandeep_crv_2016}, and recent work on learning to predict camera views \citep{Jayaraman2017}. An overview of classic AL methods \citep{settles2012active}.

We experimented with using Gaussian Process based upper confidence bounds on both the cross-entropy $H\left[e(I_x) \| \theta_x\Phi \right]$ and $D_{KL}\left[e(I_x)_x \| \theta_x\Phi\right]$ as well as the predicted topic-prior entropy $H\left[\theta(x)\right]$ as acquisition functions determining the next-best sample location. We applied these acquisition functions to both the view-prediction problem and training a supervised classifier, however found that neither outperformed a uniform sampling strategy.