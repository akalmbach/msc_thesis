...
Similar to previous examples in this thesis, we expect that each community should be mainly comprised of only a few taxa, that for any location few communities should be present, and that within some neighborhood the distribution of communities should be similar. By applying priors which prefer models that respect these conditions, we ensure that our community model will be not only accurate, but also easy to interpret.  As we have seen, the (spatio-) temporal topic model can be used to estimate the posterior probability of topic assignments $P(\boldsymbol{z} | \boldsymbol{w})$ via the collapsed Gibbs sampler update (in Eqn.~\ref{eqn:posterior}), and further that given a set of topic assignments, the posterior distributions for $\Theta$ and $\Phi$ are given by $Dir(\alpha + \boldsymbol{N}_{g(x)}^{1:K})$ and $Dir(\beta + \boldsymbol{N}_k^{1:V})$ respectively. Thus, the maximum likelihood estimates for $\Theta$ and $\Phi$ are identical to those used in the predictive distribution, and \emph{maximum a posteriori} (MAP) estimates $\hat{\Theta}, \hat{\Phi}$ can be easily computed as well:
\begin{equation} \label{eqn:mle_map_thetaphi}
\begin{aligned}
\mathbb{E}[\Theta_{g(x),k} | \boldsymbol{z}] &= \frac{N^k_{g(x)} + \alpha}{\sum_{j=1}^K N^j_{g(x)} + \alpha},
\quad &\hat{\Theta}_{g(x),k} &= \frac{N^k_{g(x)} + \alpha - 1}{\sum_{j=1}^K N^j_{g(x)} + \alpha - 1}\\
\mathbb{E}[\Phi_{k,w_i} | \boldsymbol{z}] &= \frac{N^{w_i}_k + \beta}{\sum_{v=1}^V N^v_k + \beta},
\quad &\hat{\Phi}_{k,w_i} &= \frac{N^{w_i}_k + \beta - 1}{\sum_{v=1}^V N^v_k + \beta - 1}
\end{aligned}
\end{equation}
\todo[inline]{The MAP estimate assumes that $N + \alpha > 1$, however if $N=0$ we set the probability to 0 and disregard this dimension in the summation. Where to put this comment?}

To tie our communities back to the domain of phytoplankton observations, we consider learning the community model, but also additional means of estimating the MAP community distributions when the phytoplankton observations themselves are not available. Given some estimate of MAP community distributions $\check{\theta} \approx \hat{\theta}$, we can in turn estimate the distribution of phytoplankton taxa
...
We evaluated the resulting regression systems for all hyperparameter settings as well as the two baseline methods by comparing the predicted taxon distributions to the true distributions on each day in the dataset. Our error measure is the KL-Divergence between the predictions and the held-out distributons. We chose the community model with the lowest average KL-Divergence over all the days in the dataset, ultimately picking a model with 6 active communities. The predicted taxon distributions for this model are shown in Fig.~\ref{fig:plankton-mvco-topic-est}.
...
\begin{figure}
	\centering
	\includegraphics[width=0.95\textwidth]{figures/oceans/timeseries.png}
...
Then given this training set, we learn the topic assignments and MAP priors $\hat{\Theta}$ and $\hat{\Phi}$ as in the other examples in this thesis. At
test-time we disregard the target taxon, assuming it is falsely-missing. To address this we consider the topics excluding $v^\star$ to be the MAP value of the Dirichlet over the other $V-1$ taxa
\begin{equation}
P(w = v | z = k, \boldsymbol{w}) \approx \hat{\Phi}_k^{\neg v^\star} \triangleq \frac{N_k^v + \beta - 1}{\sum_{u\neq v^\star} N_k^u + \beta - 1}
\end{equation}

Next, we obtain topic assignments $\boldsymbol{z_{\neg v^\star}}$ by substituting $\hat{\Phi}_k^{\neg v^\star}$ for $\Phi$ in the predictive distribution Eqn.~\ref{eqn:posterior}. These topic assignments allow us to compute an approximation of the true MAP topic priors,
\begin{equation}
\check{\Theta}_{g(x),k} = \frac{N^k_{g(x)} + \alpha - 1}{\sum_{j=1}^K N^j_{g(x)} + \alpha - 1}
\end{equation}
with the counts now being over $\boldsymbol{z_{\neg v^\star}}$ in place of the true topic assignments $\boldsymbol{z}$.
Finally, we estimate the proportion of the data which would have been made of the target class using the original MAP topic matrix
as $\sum_{k=0}^K \check{\Theta}_{.,k} \hat{\Phi}_{k,v^\star}$ .
